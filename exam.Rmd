...
# A lot of NAs in gross -> we'll need to address them
...
# Since our Genre entries are composed by comma-separated values, we need to split them, one-hot encode them, and remove the original column since 
...
# prices - cumulative price increases (in percentages) from 1920 to 2020, sorted

# Checkpoint - dataset without inflation correction and with
...
#sapply(moviesC, function(x) sum(is.na(x)))
...
#   geom_point() +
#   scale_x_continuous(name = "Released year", breaks = seq(1920, 2020, 10)) +
#   scale_y_continuous(name = "Gross", trans = "log") +
#   theme_bw()
...
A questo punto, con una punta di clichè, concludiamo che </br>
...
moviesCShuf = grossPerRating[sample(nrow(grossPerRating)), ]

# Number of folds to use - usually people go with 5 or 10
...
folds = cut(seq(1, nrow(grossPerRating)), breaks = K, labels = FALSE)
...
for(i in 1:K){
    ...
    testData = moviesCShuf[testIndexes, ]
    trainData = moviesCShuf[-testIndexes, ]
...
# What about people and long movies? Do they like them?
# Other Ricky Gervais quote made me wonder
...
ggplot(ratingPerRuntime, aes(x = Runtime, y = meanRating)) + 
             ggtitle("Distribution of rating over runtime") + 
             geom_point() + scale_x_continuous(name = "Runtime", breaks = seq(0, 350, 50)) + 
             scale_y_continuous(name = "Rating") + 
             theme_bw()
...
# But, are longer movies better on average?
...
# Using a log scale because gross was smahsed against x axis
